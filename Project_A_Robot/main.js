...
var roads = [
  ...
  function buildGraph(edges) {            //edges input is roads variable above
    ...
    //uses .map to create array and .split to separate "Start-End" format to [from, to] and iterates through roads variable
      ...
      if (!roadGraph[this.place].includes(destination)) {   
        ...
        let parcels = this.parcels
          .map(p => {   
            ...
            //this maps a new array of objects(parcels) with the place = destination(where the robot is moving to)
            return { place: destination, address: p.address };  
          })
          // filters out objects where the place and address are equal (dropping off parcels)
          .filter(p => p.place != p.address);          
          
          // returns new class (state) with updated destination (state.place) and parcels (state.parcels)
  ...
  function runRobot(state, robot, memory) {
    for (let turn = 0; ; turn++) {            //iterates forever until there are no objects in parcels array
      ...
      // robot's .direction and .memory are generated by type of robot function, input into VillageState.move()
  ...
  VillageState.random = function(parcelCount = 2) {         //the map starts with 5 parcels
    let parcels = [];
  ...
  function findRoute(graph, from, to) {   //from is current robot.place, to is either parcels.place or parcels.address
    ...
    for (let i = 0; i < work.length; i++) {
      let { at, route } = work[i];
       ...
       //iterates through values of roadGraph's property(robot's current location)
      for (let place of graph[at]) {     

        //if one of the values matches parcel's (either place or address) it's added to route
        if (place == to) return route.concat(place);    

        // if no object in work array has .at value equal to place
  ...
  //in runRobot function, robot (state, memory), {place, parcels} = state, route = memory
  function goalOrientedRobot({ place, parcels }, route) {   
    if (route.length == 0) {

         //starts with first parcel robot has
      let parcel = parcels[0];   

      //if the parcel was not just picked up at the current place
      if (parcel.place != place) {      

        //run findRoute with to = parcel's current place
        route = findRoute(roadGraph, place, parcel.place);    
      } else {

        // if the parcel's current place is robots place, findRoute with parcel's address (where its going)
        route = findRoute(roadGraph, place, parcel.address);  
      }
    } //console.log(route);
    // /direction = first value in roadGraph's from(robot's current location), 
    // memory = removes first item from the array. when array is empty above code executes.
